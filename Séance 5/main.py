...
freq_population = {
    k: round(v / population_totale, 2)
    for k, v in population.items()
}

df = ouvrirUnFichier("data/Echantillonnage-100-Echantillons.csv")

moyennes = df.mean().round(0)

total_moyennes = moyennes.sum()
freq_echantillons = round(moyennes / total_moyennes, 2)

print("Fréquences population mère :", freq_population)
print("Fréquences échantillons :", freq_echantillons.to_dict())

z = 1.96

def intervalle_fluctuation(p, n):
    ecart_type = math.sqrt((p * (1 - p)) / n)
    return (round(p - z * ecart_type, 4), round(p + z * ecart_type, 4))

n = int(total_moyennes)

intervals_fluctuation = {
    col: intervalle_fluctuation(freq_population[col], n)
    for col in freq_population
}

print("Intervalles de fluctuation :", intervals_fluctuation)
...
ligne_1 = list(df.iloc[0])

n_echantillon = sum(ligne_1)

freq_obs = [x / n_echantillon for x in ligne_1]

def intervalle_confiance(p, n):
    ecart_type = math.sqrt((p * (1 - p)) / n)
    return (round(p - z * ecart_type, 4), round(p + z * ecart_type, 4))

ic = [intervalle_confiance(p, n_echantillon) for p in freq_obs]
...
print("Fréquences observées :", freq_obs)
print("Intervalles de confiance :", resultats_ic)
...
data1 = pd.read_csv("data/Loi-normale-Test-1.csv").iloc[:, 0]
data2 = pd.read_csv("data/Loi-normale-Test-2.csv").iloc[:, 0]

stat1, p1 = shapiro(data1)
stat2, p2 = shapiro(data2)

print("Test 1 : statistique =", stat1, "p-value =", p1)
print("Test 2 : statistique =", stat2, "p-value =", p2)

alpha = 0.05

if p1 > alpha:
    print("Loi-normale-Test-1 suit une loi normale")
else:
    print("Loi-normale-Test-1 ne suit pas une loi normale")

if p2 > alpha:
    print("Loi-normale-Test-2 suit une loi normale")
else:
    print("Loi-normale-Test-2 ne suit pas une loi normale")
